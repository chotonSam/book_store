...
const Book = ({ book, books }) => {
  ...
  const { setBookData, SeteditId } = context;
  ...
  for (let i = 0; i < star; i++) {
    itemsToRender.push(<Star key={Math.random()} />);
  }

  const editHandler = () => {
    let newEditItem = books.find((el) => {
      return el.id === id;
    });
    SeteditId(newEditItem.id);
    setBookData(newEditItem);
  };
    ...
    <div className="book-card">
      ...
      <div className="flex-1 h-full pr-2 pt-2 flex flex-col">
        ...
        <div className="space-y-2 mt-4 h-full">
          ...
          <div className="stars">{itemsToRender}</div>
